...
const linkIcons = [Linkedin, Github, FileText, FileText];
  ...
  return (
    <section id="contato" className="py-12 md:py-16 px-4 md:px-6 lg:px-8">
      <div className="max-w-[980px] mx-auto">
        ...
        <motion.a
          href="mailto:almeida@korvenlab.com"
          ...
          className="group block bg-[#0A0A0A]/90 backdrop-blur-sm rounded-3xl p-5 md:p-6 mb-3 md:mb-4 transition-all cursor-pointer text-center border-white/20 no-underline"
          style={{ borderWidth: 1, borderColor: "rgba(255, 255, 255, 0.2)" }}
        >
          <motion.div animate={{ rotate: isHovering ? [0, -10, 10, 0] : 0 }} transition={{ duration: 0.5 }}>
            <Mail className="w-7 h-7 md:w-8 md:h-8 text-[#A855F7] mx-auto mb-2 md:mb-3" />
          </motion.div>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 md:gap-3">
            <h3
              className="text-lg md:text-2xl text-white transition-colors"
              style={{
                fontFamily: "'Fraunces', Georgia, serif",
                fontWeight: 600,
                letterSpacing: "-0.02em",
        ...
        >
          {t.contact.links.map((link, index) => {
            const Icon = linkIcons[index] ?? Github;
            return (
              <motion.a
                key={link.platform}
                href={link.url}
